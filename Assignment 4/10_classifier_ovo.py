...
for i, row in test_df.iterrows():
    ...
    for (c1,c2), svm in svm_classifiers.items():
        if svm.predict([row.values])==c1:
            ...
            votes[np.where(classes==c2)]+=1
    y_pred_test_ovo.append(classes[np.argmax(votes)])

y_pred_test_ovo = le.inverse_transform(y_pred_test_ovo)
#should output predicted labels in a new file named ovo.csv
pd.DataFrame({"predicted":y_pred_test_ovo}).to_csv("ovo.csv", header=True, index=None)